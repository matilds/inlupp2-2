  ...
  else
    {
      ...
      *insert_place = new_node;
      ...
      if(abs(node_depth(tree->root->right) - node_depth(tree->root->left)) <= 1)
        ...
        {
          node_t *unbalanced_part = calloc(1,sizeof(node_t));/* TODO:  *//* TODO:  *//* TODO:  */
          do
        {
...
void print_num_iteration(int num)
{
  int x = 1;
  while (x <= num)
    {
...
void print_num_rekursion(int num , int current_num)
{
  if( current_num <= num)
    {
      printf("%d\n" , current_num);
      print_num_rekursion(num , current_num + 1);
    }
  else
    {
      return;
...
void ekvivalenta()
{
  int b = 1;
  int a = 1;
  if ( &a == &b) printf("a är ekvivalent med b\n");
  else puts("a är inte ekvivalent med b");
  if( a == b) puts("a och b är identiska");
  else puts("a är inte identisk med b");
}
...
int main()
{
  /*
  puts("iteration");
  print_num_iteration(5);
  puts("rekursion");
  print_num_rekursion(5,1);
  puts("\nidentiska?");
  identical();
  puts("\nekvivalenta?");
  ekvivalenta();
